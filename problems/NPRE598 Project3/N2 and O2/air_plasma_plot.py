...
filename = 'air_plasma_case4'
#titlename = 'air_plasma: 50-50 N2-O2'
titlename = '50-50 N2-O2'
typename = np.array(('Neutral','Positive','Negative'))
...
for i in range(0,2):
    ...
    if i ==0:
        ...
        plt.legend(loc='upper left', ncol=3, framealpha=1.0, prop={'size': 9})
        plt.title(titlename)
        plt.grid()
        plt.tight_layout()
...
for i in range(0,3):
    ...
    plt.legend(loc='best', ncol=3, framealpha=1.0, prop={'size': 9})
    plt.title(typename[i]+' Density vs Time for '+titlename)
    plt.grid()
    plt.tight_layout()
    #plt.savefig('air plasma ('+typename[i]+'for '+titlename+').png',dpi=300)
    #plt.savefig(filename+', log-scale.png',dpi=300)
plt.show()
